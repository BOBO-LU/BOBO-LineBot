...
def update_googlesheet(url='none'):
    ...
    #選擇gsheet
    workbook = gc.open_by_url("https://docs.google.com/spreadsheets/d/1UL7R-E4kHF1ZqsnOcTi6uIw-I4BzfRyGF9gPh9-FKYk/edit?usp=sharing")
    print("workbook:", workbook)
    print("workbook.url:", workbook.url)

    # 選擇追蹤清單
    work_sheet = workbook.worksheet_by_title("工作表1")
    print("work_sheet:", work_sheet)

    # 偵測該工作表最後一個 row
    col_a_data = work_sheet.get_col(1, include_tailing_empty=False)
    last_row = str(len(col_a_data)+1)
    print(last_row)

    # wks.insert_rows(row=last_row, values=['https'])
    location_title = 'A'+last_row
    location_url = 'B'+last_row
